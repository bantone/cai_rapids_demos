...
#****************************************************************************
# Transaction generator - v2
#
# Differences from 01_generate_transactions.py:
#   - spark.range() is given an explicit numPartitions so partition size
#     doesn't grow unbounded as row count increases.
#   - rows default raised to 100,000,000.
#
# createSparkConnection() is unchanged from the original -- it still goes
# through cmldata.get_connection().get_spark_session(), which is required
# for S3A/IDBroker write auth on this database's storage. (A version of
# this file that built the SparkSession directly to work around the
# SparkContext.setSystemProperty()-runs-too-late issue was tried and
# failed with "Authentication with IDBroker failed" on saveAsTable,
# since that auth setup lives inside get_spark_session() and has no
# equivalent in a plain SparkSession.builder session.)
#****************************************************************************
...
class TransactionFactGenerator:
    ...
    def generateTransactions(
        ...
        transactions = (
            ...
            spark.range(rows, numPartitions=2000)
